...
const Layout = ({ children }: any) => {
  ...
  useEffect(() => {
    ...
    document.body.classList.add(
      ...
      "group-data-[skin=bordered]:dark:bg-zink-700"
    );
    return () => {
  ...
  return (
    <>
      ...
      {layoutNavigationType === "hidden" && (
        <div className="fixed items-center bottom-6 right-12 h-header group-data-[navbar=hidden]:flex">
          <button
            type="button"
            className="inline-flex items-center justify-center size-12 p-0 transition-all duration-200 ease-linear rounded-md shadow-lg text-sky-50 bg-sky-500"
