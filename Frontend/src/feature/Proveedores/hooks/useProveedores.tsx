...
import { CreaterProveedores, DeletedProveedores, ProveedoresService } from 'services/service.cruds/Proveedores/Proveedores';
import { ProveedorInterface } from 'interfaces/Proveedor/Proveedor';
import { useProveedoresStore } from '../store';
import { UpdateGuarnicion } from 'services/service.cruds/Guarnicion';
...
export const useProveedores = () => {
    ...
    const { setIsEdit, entity, isEdit, setIdToDelete, setEntity, idToDelete } = useProveedoresStore();
    ...
    const initialValues = {
        ...
        telefono: entity?.telefono || "",
        direccion: entity?.direccion || "",
        correo: entity?.direccion || "",
        tipoProducto: entity?.direccion || "",
    ...
    const validationSchema = Yup.object({
        ...
        correo: Yup.string().required("El nombre es obligatorio"),
        tipoProducto: Yup.string().required("El nombre es obligatorio"),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (isEdit) {
                if (entity) {
                    onUserUpdate(entity.idProveedor || 0, payload);
                } else {
    ...
    const create = async (payload: any) => {
        try {
            ...
            const response = await CreaterProveedores(payload)
    ...
    const createRows = (proveedor: any) => {
        ...
        const rows = proveedor.map((proveedor: ProveedorInterface) => ({
            ...
            actions: (
                <div className="flex gap-2">
                    <button
                        onClick={() => onClickOpenForm(proveedor)}
                    ...
                    <button
                        onClick={() => onclickDelete(proveedor)}
    ...
    const onclickDelete = (user: ProveedorInterface) => {
        setShowDeleteModal(true);
        setIdToDelete(user.idProveedor || 0);
    };

    const onClickOpenForm = (user: ProveedorInterface) => {
        setIsEdit(true);
        setEntity(user);
    };
    ...
    const onDeleteConfirm = async () => {
        try {
            ...
            const responseDelete = await DeletedProveedores(idToDelete);
